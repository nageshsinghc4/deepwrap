...
def is_classifier(model):
    ...
    if loss == 'binary_crossentropy':
        if is_huggingface(model=model):
            ...
            if ((hasattr(last, 'activation') and isinstance(last.activation, type(sigmoid))) or isinstance(last, type(
                    sigmoid))) and mult_output:
                is_multilabel = True
...
def check_array(X, y=None, X_name='X', y_name='targets'):
    ...
    if y is not None and not isinstance(y, (list, np.ndarray)): raise ValueError(
        "%s must be a list or NumPy array" % y_name)
    return
...
class YTransform:
    ...
    def apply(self, targets, train=True):
        ...
        # string targets (classification)
            ...
            if not train and self.le is None: raise ValueError(
                'LabelEncoder has not been trained. Call with train=True')
            if train:
                self.le = LabelEncoder()
                self.le.fit(targets)
                if self.get_classes():
                    warnings.warn('class_names argument was ignored, as they were extracted from string labels in '
                                  'dataset')
                self.set_classes(self.le.classes_)
            targets = self.le.transform(targets)  # convert to numerical targets for classification
        ...
        if len(targets.shape) == 1 and self.get_classes():
            if np.issubdtype(type(max(targets)), np.floating):
                warnings.warn(
                    'class_names implies classification but targets array contains float(s) instead of integers or '
                    'strings')

            # TODO check logic
            if train and (len(set(targets)) != len(list(range(int(max(targets) + 1))))):
                raise ValueError('len(set(targets) is %s but len(list(range(int(max(targets)+1))) is  %s' % (
                    len(list(set(targets))), len(list(range(int(max(targets)) + 1)))))
...
class YTransformDataFrame(YTransform):
    ...
    def apply(self, df, train=True):
        ...
        if len(self.label_columns) > 1:
            ...
            # set targets
            targets = df[self.label_columns].values if labels_exist else np.zeros(
                (df.shape[0], len(self.label_columns)))
            # set class names
