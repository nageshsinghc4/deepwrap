from .imports import *
from .optimizer.sgdr import *
from .optimizer.triangular import *
from .optimizer.lrfinder import *
from .optimizer.optimization import AdamWeightDecay
...
class Learner(ABC):
    ...
    def _check_val(self, val_data):
        if val_data is not None:
            ...
            val = self.val_data
        if val is None:
            raise Exception('val_data must be supplied to get_learner or view_top_losses')
        return val
    ...
    def _make_model_folder(self, fpath):
        if os.path.isfile(fpath):
            raise ValueError(f'There is an existing file named {fpath}. ' + \
                             'Please use different value for fpath.')
    ...
    def _recompile(self, wd=None):
        ...
        if wd is not None and wd > 0 and type(self.model.optimizer).__name__ != 'AdamWeightDecay':
            warnings.warn('recompiling model to use AdamWeightDecay as optimizer with weight decay of %s' % (wd))
    ...
    def lr_find(self, start_lr=1e-7, lr_mult=1.01, max_epochs=None, class_weight=None,
        ...
        U.vprint('simulating training for different learning rates... this may take a few moments...',
                 verbose=verbose)
        # save current weights and temporarily restore original weights dep_fix: temporarily use save_model instead
        # of save_weights as default due to https://github.com/tensorflow/tensorflow/issues/41116
        _weights_only = True
        ...
        # re-load current weights dep_fix: temporarily use load_model instead of load_weights as default due to
        # https://github.com/tensorflow/tensorflow/issues/41116
        if restore_weights_only:
            self.model.load_weights(weightfile)
        else:
            self.load_model(temp_folder)
        ...
        if show_plot:
            U.vprint('Visually inspect loss plot and select learning rate associated with falling loss',
                     verbose=verbose)
            self.lr_plot()
        else:
    ...
    def autofit(self, lr, epochs=None,
        ...
        if reduce_on_plateau is not None or early_stopping is not None:
            if monitor.startswith('val_') and self.val_data is None:
                raise ValueError(
                    'monitor is %s but no val_data was supplied.\nChange monitor or supply val_data to get_learner '
                    'function.' % monitor)
...
def load_predictor(fpath, batch_size=U.DEFAULT_BS):
    ...
    try:
        ...
        try:
            ...
            # warnings.warn('could not load .preproc file as %s - attempting to load as %s' % (os.path.join(fpath,
            # U.PREPROC_NAME), preproc_name))
            with open(preproc_name, 'rb') as f:
                preproc = pickle.load(f)
        except:
            raise Exception(
                'Could not find a .preproc file in either the post v0.16.x loction (%s) or pre v0.16.x location (%s)' % (
...
def _load_model(fpath, preproc=None, train_data=None, custom_objects=None):
    ...
    if (preproc and isinstance(preproc, TransformersPreprocessor)) or \
        ...
        return model
    elif (preproc and (isinstance(preproc, BERTPreprocessor) or type(preproc).__name__ == 'BERTPreprocessor')) or \
    ...
    try:
        try:
            ...
            try:
                # pre-0.16: model fpath was file name of model not folder for non-Transformer models warnings.warn(
                # 'could not load model as %s - attempting to load model as %s' % (os.path.join(fpath, U.MODEL_NAME),
                # fpath))
